...
%   DETECTDARKLESIONS, DETECTNEOVASCULARIZATION.
    ...
    % The one lesion channel fit to display: recall 0.254, precision 0.595.
    % It under-detects rather than over-detects, which is the correct direction
    % of error when a clinician reads the output.
    F.hardExudates = struct( ...
        'reliable', true, ...
        'measuredRecall', 0.254, ...
        'measuredPrecision', 0.595, ...
...
%   silently teach it that ungradable images are healthy.
    ...
    F.hardExudates = struct('reliable', true, 'measuredRecall', 0.254, ...
        'measuredPrecision', 0.595, 'areaDD2', NaN, 'count', NaN, ...
